...
- **`packages/runtime`** — inference. Default = **W&B Inference** (OpenAI-compatible) driven by
  the **OpenAI Agents SDK**; OpenAI switchable via `RUNTIME_PROVIDER`.
...
pnpm --filter @weavehacks/api agent:check   # prove the runtime (OpenAI Agents SDK over W&B Inference)
```

Requirements: Node 20+, pnpm, Docker (for Redis; otherwise bring your own `REDIS_URL`).
Copy `.env.example` → `.env` and fill in `WANDB_API_KEY` — one key powers both Weave and W&B
Inference. The deterministic scoreboard (`pnpm compare`/`demo`) runs with no keys at all.
